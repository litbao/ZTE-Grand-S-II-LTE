...
#ifdef CONFIG_POWERSAVE_MODE
static uint powersave_limit = 0;
static uint flag_for_powersave = 0;

module_param(powersave_limit, uint, 0664);
module_param(flag_for_powersave, uint, 0664);

static unsigned int user_cap_speed(unsigned int requested_speed)
{
	if ((powersave_limit) && (requested_speed > powersave_limit)) {
		pr_debug("===tengfei powersave: limiting freq to %d ===\n",powersave_limit);
		return powersave_limit;
	}		
