...
#ifdef CONFIG_ANDROID_RAM_CONSOLE_LEGACY
...
static int __init ram_console_device_init(void)
{
	int err;
...
static void __init ram_console_reserve(unsigned int size)
{
	struct resource *res;
	long ret;
...
#else
static int __init ram_console_device_init(void) { return 0; }
static void __init ram_console_reserve(unsigned long size) {}
#endif

#ifdef CONFIG_BOOTLOADER_LOGGER
static struct resource bootloader_logger_resources[] = {
	{
		.flags = IORESOURCE_MEM,
	},
};
static struct platform_device bootloader_logger_device = {
	.name 		= "bootloader_logger",
	.id 		= -1,
	.num_resources	= ARRAY_SIZE(bootloader_logger_resources),
	.resource	= bootloader_logger_resources,
};
...
static void __init bootloader_logger_init(void)
{
	int err;

	err = platform_device_register(&bootloader_logger_device);
	if (err) {
		pr_err("%s: bootloader_logger registration failed (%d)!\n", __func__, err);
	}
}
#else
static void __init bootloader_logger_reserve(int size) {}
