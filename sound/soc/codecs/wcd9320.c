...
/*ZTE: by hunan: add for micbias1 for A23, begin */
static u8 micbias1_enable = 0;
static int taiko_get_micbias1(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = micbias1_enable;
	return 0;
}
    
static int taiko_set_micbias1(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct taiko_priv *taiko = snd_soc_codec_get_drvdata(codec);    
	...
	cfilt_sel_val = taiko->resmgr.pdata->micbias.bias1_cfilt_sel;
        ...
        micbias1_enable = ucontrol->value.integer.value[0];
        pr_debug("%s: micbias1_enable=%x\n", __func__, micbias1_enable);
	if (micbias1_enable){
		/* Get cfilt */
		snd_soc_update_bits(codec, WCD9XXX_A_MICB_CFILT_1_CTL, 0x80, 0x80);

		snd_soc_update_bits(codec, TAIKO_A_MICB_1_CTL, 1 << 7, 1 << 7);
        
		usleep_range(20000, 20000);
	}
        else{
		snd_soc_update_bits(codec, TAIKO_A_MICB_1_CTL, 1 << 7,	 0);

		/* Put cfilt */
                snd_soc_update_bits(codec, WCD9XXX_A_MICB_CFILT_1_CTL, 0x80, 0);
        }
	return 0;
}
/*ZTE: by hunan: add for micbias1 for A23, end */
/*ZTE: by wangtao: add for micbias1 enable for A23, begin */
static int taiko_get_micbias1_statue(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = micbias1_enable;
	return 0;
...
static int taiko_set_micbias1_statue(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

        micbias1_enable = ucontrol->value.integer.value[0];
	VS_statue = ucontrol->value.integer.value[0];
        pr_err("%s: micbias1_statue=%x,VS_statue=%x\n", __func__, micbias1_enable,VS_statue);
	if (micbias1_enable){
		snd_soc_update_bits(codec, WCD9XXX_A_BIAS_CENTRAL_BG_CTL, 0x51, 0x51);
		snd_soc_update_bits(codec, WCD9XXX_A_CLK_BUFF_EN1, 0x04, 0x04);
		snd_soc_update_bits(codec, WCD9XXX_A_LDO_H_MODE_1, 0xed, 0xed);
		snd_soc_update_bits(codec, WCD9XXX_A_MICB_CFILT_1_CTL, 0xc0, 0xc0);
		snd_soc_update_bits(codec, WCD9XXX_A_MICB_1_CTL, 0x80, 0x80);
		usleep_range(20000, 20000);
	}else{
		if(!enable_bg)
			snd_soc_update_bits(codec, WCD9XXX_A_BIAS_CENTRAL_BG_CTL, 0x51, 0x50);
		snd_soc_update_bits(codec, WCD9XXX_A_CLK_BUFF_EN1, 0x04, 0x04);
		//snd_soc_update_bits(codec, WCD9XXX_A_LDO_H_MODE_1, 0xed, 0x65);//c for mic mute in call after VS
		snd_soc_update_bits(codec, WCD9XXX_A_MICB_CFILT_1_CTL, 0xc0, 0x40);
		snd_soc_update_bits(codec, WCD9XXX_A_MICB_1_CTL, 0x80, 0x00);
        }
	return 0;
}
/*ZTE: by wangtao: add for micbias1 enable for A23, end */
...
static const struct snd_kcontrol_new taiko_snd_controls[] = {
	...
	SOC_ENUM_EXT("MAD Input", taiko_conn_mad_enum,
        ...
        /*ZTE: by hunan: add for micbias1 for A23, begin */
        SOC_SINGLE_EXT("MIC BIAS1 External", SND_SOC_NOPM, 0, 1, 0,
                       taiko_get_micbias1, taiko_set_micbias1),
        /*ZTE: by hunan: add for micbias1 for A23, end */        
        /*ZTE: by wangtao: add for micbias1 enable for A23, begin */
        SOC_SINGLE_EXT("MIC BIAS1 Switch", SND_SOC_NOPM, 0, 1, 0,
                       taiko_get_micbias1_statue, taiko_set_micbias1_statue),
        /*ZTE: by wangtao: add for micbias1 enable for A23, end */
};
...
static const struct snd_soc_dapm_route audio_map[] = {
	...
	{"ANC2 MUX", "ADC3", "ADC3"},
	{"ANC2 MUX", "ADC4", "ADC4"},
        /* ZTE: by hunan begin: add for DMIC*/
