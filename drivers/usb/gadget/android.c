...
#define MAX_MASS_STORAGE_NLUNS	4
					...
					struct usb_composite_dev *cdev)
{
	...
	config->fsg.nluns = MAX_MASS_STORAGE_NLUNS;
	for (i = 0; i < MAX_MASS_STORAGE_NLUNS; i++)
						...
						struct usb_configuration *c)
{
	...
	config->common->nluns = config->nluns;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%d", &value) == 1) {
		if (value > MAX_MASS_STORAGE_NLUNS)
			value = MAX_MASS_STORAGE_NLUNS;
		if (value < 1)
			value = 1;
		config->nluns = value;
		return size;
...
static int cdrom_function_ctrlrequest(struct android_usb_function *f,
						struct usb_composite_dev *cdev,
						const struct usb_ctrlrequest *c)
{
	int	value = -EOPNOTSUPP;
	char *uevent_envp[] = { "OPERATION=STOP", NULL };

	if ((c->bRequestType == 
			(USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE))
		 && (c->bRequest == 0xa1)) {
		pr_info("%s(%d): Got ZTE vendor CDROM eject command.\n", __func__, __LINE__);
		kobject_uevent_env(&f->dev->kobj, KOBJ_CHANGE, uevent_envp);
		value = 0;
	}
...
static struct android_usb_function cdrom_function = {
	...
	.ctrlrequest	= cdrom_function_ctrlrequest,
	.attributes	= mass_storage_function_attributes,
};
