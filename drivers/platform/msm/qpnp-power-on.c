...
/**
0x808  PON_PON_REASON1
0x80A  PON_WARM_RESET_REASON1 
0x80B  PON_WARM_RESET_REASON2 
0x80C  PON_POFF_REASON1 
0x80D  PON_POFF_REASON2 
0x80E  PON_SOFT_RESET_REASON1 
0x80F  PON_SOFT_RESET_REASON2 
*/
static void
qpnp_pon_read(struct qpnp_pon *pon, u16 addr, u8 *val,int count)
{
	int rc;
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	dev_info(&pon->pon_input->dev, "qpnp_pon(%d) is %s\n",
		cfg->key_code, !!(pon_rt_sts & pon_rt_bit) ? "pressed" : "released");
...
static void bark_work_func(struct work_struct *work)
{
	...
	if (!(pon_rt_sts & QPNP_PON_RESIN_BARK_N_SET)) {
		...
		dev_info(&pon->pon_input->dev, "qpnp_pon_w(%d) is released\n", cfg->key_code);
...
static irqreturn_t qpnp_resin_bark_irq(int irq, void *_pon)
{
	...
	dev_info(&pon->pon_input->dev, "qpnp_pon_i(%d) is pressed\n", cfg->key_code);
...
static int __devinit qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	qpnp_pon_read(pon, 0x808, pon_reg, 7);
	memset(buff, 0, sizeof(buff));
	for(i = 0; i < 7; i++)
		sprintf((buff + (i<<2)), "%2x  ", pon_reg[i]);
	pr_info("PON %s\n",buff);
