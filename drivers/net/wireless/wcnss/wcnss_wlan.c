...
#include <linux/proc_fs.h>  /** ZTE_MODIFY wangzhanmeng, add BT FTM command   20130901 */
...
#define	WCN_MODULE_VID_GPIO        25/** ZTE_MODIFY wangzhanmeng add wifi module vid function   20130701 */
...
/** ZTE_MODIFY wangzhanmeng add wifi module vid function   20130701 */
#define NVBIN_FILE_SAMSUNG "wlan/prima/WCNSS_qcom_wlan_nv_samsung.bin"
#define NVBIN_FILE_MURATA "wlan/prima/WCNSS_qcom_wlan_nv_murata.bin"
/** ZTE_MODIFY wangzhanmeng add wifi module vid function   end */
...
/** ZTE_MODIFY wangzhanmeng add wifi module vid function   20130701 */
static int wcn_module_vid_config(bool enable)
{
	int rc = 0;
			pr_err("===www[tp]%s ,line=%d enable=%d \n", __func__,__LINE__,enable);

		if (enable) {
			rc = gpio_request(WCN_MODULE_VID_GPIO, "wcn_module_vid");
			if (rc) {
				pr_err("WCNSS: gpio_request wcn_module_vid:25  err %d\n", rc);
				gpio_free(WCN_MODULE_VID_GPIO);
			}
			rc = gpio_direction_input(WCN_MODULE_VID_GPIO);
			if (rc) {
				pr_err("WCNSS: gpio_direction_input  wcn_module_vid:25  err %d\n", rc);
				gpio_free(WCN_MODULE_VID_GPIO);
			}
...
int wcn_module_vid_get(void)
{
	int rc;

	 rc=gpio_get_value(WCN_MODULE_VID_GPIO);
	 pr_err("WCNSS %s : wcn_module_vid= %d\n", __func__,rc);

	 return rc;
}
EXPORT_SYMBOL(wcn_module_vid_get);
/** ZTE_MODIFY wangzhanmeng add wifi module vid function   end */
...
/** ZTE_MODIFY wangzhanmeng, add BT FTM command   20130901 */
static ssize_t proc_read_ftmcod_file(char *page, char **start, off_t off, int count, int *eof,void *data)
{
	int len = 0;

	len += sprintf(page + len, "BT Information:\n");
	len += sprintf(page + len, "IC manufacturer:Qualcomm\n");
	len += sprintf(page + len, "IC name:WCN3680\n");

	if(wcn_module_vid_get())
		len += sprintf(page + len, "Module manufacturer:Samsung\n");
	else
		len += sprintf(page + len, "Module manufacturer:Murata\n");
	if (off + count >= len)
		*eof = 1;
	if (len < off)
	{
...
static int wcnss_creat_ftmcod_file(void)
{
	struct proc_dir_entry  *refresh;

	refresh = create_proc_entry("msm_btinfo", 0644, NULL);
	if (!refresh)
		return -ENOMEM;
	if (refresh) {
		refresh->data		= NULL;
		refresh->read_proc  = proc_read_ftmcod_file;
	}
	return 0;	

}
/** ZTE_MODIFY wangzhanmeng, add BT FTM command   end */
...
wcnss_ctrl_probe(struct platform_device *pdev)
{
	...
	wcnss_creat_ftmcod_file();/** ZTE_MODIFY wangzhanmeng, add BT FTM command   20130901 */
	return 0;
}
...
static void wcnss_nvbin_dnld(void)
{
	...
	char *nv_fileName;/** ZTE_MODIFY wangzhanmeng add wifi module vid function   20130701 */
...
/** ZTE_MODIFY wangzhanmeng add wifi module vid function   20130701 */
	...
	#else
	if(wcn_module_vid_get())
		nv_fileName=NVBIN_FILE_SAMSUNG;
	else
		nv_fileName=NVBIN_FILE_MURATA;
	
	ret = request_firmware(&nv, nv_fileName, dev);
	pr_err("wcnss: %s: line=%d :request_firmware for %s\n",__func__, __LINE__,nv_fileName);
...
wcnss_trigger_config(struct platform_device *pdev)
{
	...
	/** ZTE_MODIFY wangzhanmeng add wifi module vid function   20130701 */
		ret=wcn_module_vid_config(true);
		if (ret) {
		dev_err(&pdev->dev, "WCNSS Vid  gpios config failed.\n");
		goto fail_gpio_res;
	}
	/** ZTE_MODIFY wangzhanmeng add wifi module vid function   end */
	/* power up the WCNSS */
...
static int __init wcnss_wlan_init(void)
{
	...
	/** ZTE_MODIFY wangzhanmeng  test enable micro  CONFIG_WCNSS_REGISTER_DUMP_ON_BITE  20140724 */
	#ifdef CONFIG_WCNSS_REGISTER_DUMP_ON_BITE
	pr_err("CONFIG_WCNSS_REGISTER_DUMP_ON_BITE test  ....%s ,line=%d  \n", __func__,__LINE__);
	#endif
	/** ZTE_MODIFY wangzhanmeng  test enable micro  CONFIG_WCNSS_REGISTER_DUMP_ON_BITE  end */
