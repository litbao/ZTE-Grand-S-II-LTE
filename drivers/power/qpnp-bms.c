...
//#define RAISE_BATT_CAPACITY

#ifdef RAISE_BATT_CAPACITY
int show_capacity;
int trans_capacity;
bool first_trans_cap = true;

#define MAX_CAP_COLS 	21
struct qpnp_cap_map {
	int 	real_cap[MAX_CAP_COLS];
	int 	show_cap[MAX_CAP_COLS];
};

static const struct qpnp_cap_map capmap_real_show = {
	.real_cap	= {100, 99, 98, 97, 96, 95,94,93,92,91,90,89,88,87,86,85,84,83,82,81,80},
											 /*97*/   /*93*/   /*89*/    /*85*/   /*81*/
	.show_cap	= {100,100,100,100,100,100,99,98,96,95,94,92,91,90,88,87,86,84,83,82,80},
};
#endif
...
static int report_cc_based_soc(struct qpnp_bms_chip *chip)
{
	...
	#ifdef RAISE_BATT_CAPACITY
	if ((first_trans_cap)&& (chip->last_soc != -EINVAL))
	{
		show_capacity = chip->last_soc;
		first_trans_cap = false;
	}
	#endif
...
out:
	pr_info("ibat_ua = %d, vbat_uv = %d, ocv_est_uv = %d, pc_est = %d, soc_est = %d, n = %d, delta_ocv_uv = %d, last_ocv_uv = %d, pc_new = %d, soc_new = %d, rbatt = %d, slope = %d\n",
		ibat_ua, vbat_uv, ocv_est_uv, pc_est,
		soc_est, n, delta_ocv_uv, chip->last_ocv_uv,
		pc_new, soc_new, params->rbatt_mohm, slope);//pr_debug - > pr_info
...
static int clamp_soc_based_on_voltage(struct qpnp_bms_chip *chip, int soc)
{
	...
	if (soc == 0 && vbat_uv > chip->v_cutoff_uv) {
		...
		return 1;
	}else if (vbat_uv < 3000000){
		pr_info("forcing soc to 0\n");
		return 0;
	} else {
...
#ifdef RAISE_BATT_CAPACITY
static int trans_real_to_show_cap(struct qpnp_bms_chip *chip, int real_capacity)
{
	int i = 0;
	
	pr_info("real = %d, show = %d\n",real_capacity,show_capacity);
	if (real_capacity < 80)
	{
	...
	if(is_battery_charging(chip))
	{
		if(show_capacity > real_capacity)//eg: 90%插充电器,90%->91%时间会很长
			return show_capacity;
		else
			return real_capacity;//充电是真实
			
	}else
	{
		for (i = 0; i < MAX_CAP_COLS; i++)
		{
			if(real_capacity == capmap_real_show.real_cap[i])
				trans_capacity = capmap_real_show.show_cap[i];
		}
		if((show_capacity - trans_capacity ) >= 1)//放电跳变
		{
			show_capacity--;
		}
		/*else if (show_capacity < capmap_real_show.trans_cap)//eg:充到95%拔充电器
		{
			return show_capacity;
		}*/
		return show_capacity;
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
			...
			if(!first_trans_cap)
			{
			    show_capacity = trans_real_to_show_cap(chip,get_prop_bms_capacity(chip));
				val->intval = show_capacity;
			}else
			{
