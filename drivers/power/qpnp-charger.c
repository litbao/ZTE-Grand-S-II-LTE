...
#define SPEED_99P_CHG_TIME 
unsigned int CHARGE_VOLATE;
...
qpnp_chg_usb_chg_gone_irq_handler(int irq, void *_chip)
{
	...
	pr_debug("in qpnp_chg_usb_chg_gone_irq_handler:psy changed usb_psy\n");
		...
		if (ret.intval <= 2 && !chip->use_default_batt_values &&
			...
			if (((ret.intval / 1000) > USB_WALL_THRESHOLD_MA)
				...
				if (chip->therm_lvl_sel != 0 && chip->thermal_mitigation)
					ret.intval = min(ret.intval,(int)chip->thermal_mitigation[chip->therm_lvl_sel] * 1000);
...
static void
qpnp_chg_set_appropriate_iusb_current(struct qpnp_chg_chip *chip)
{
	...
	pr_debug("setting %d mA\n", chg_current);
	if (chg_current <= 2 &&	get_prop_batt_present(chip)) {
		qpnp_chg_usb_suspend_enable(chip, 1);
		qpnp_chg_iusbmax_set(chip, QPNP_CHG_I_MAX_MIN_100);
	} else {
		qpnp_chg_usb_suspend_enable(chip, 0);
		qpnp_chg_iusbmax_set(chip, chg_current);
	}
}
...
qpnp_batt_system_temp_level_set(struct qpnp_chg_chip *chip, int lvl_sel)
{
	if (lvl_sel >= 0 && lvl_sel < chip->thermal_levels) {
		...
		if (lvl_sel == (chip->thermal_levels - 1)) {
			...
			qpnp_chg_set_appropriate_iusb_current(chip);
		}
	} else {
...
qpnp_eoc_work(struct work_struct *work)
{
	...
	if (!qpnp_chg_is_usb_chg_plugged_in(chip) &&
			!qpnp_chg_is_dc_chg_plugged_in(chip)) {
			#ifdef SPEED_99P_CHG_TIME
				if(chip->max_voltage_mv != CHARGE_VOLATE){
					chip->max_voltage_mv = CHARGE_VOLATE;
				}
				pr_info("1.nochg max_vol: %d\n",(chip->max_voltage_mv));
			#endif
	...
	if ((batt_sts & BAT_FET_ON_IRQ) && (chg_sts & FAST_CHG_ON_IRQ
		...
		{
			#ifdef SPEED_99P_CHG_TIME
			if(get_prop_capacity(chip) == 98){
				if(chip->max_voltage_mv != 4360){
					chip->max_voltage_mv = 4360;
				}
			}else if(get_prop_capacity(chip) >= 99){
				if(chip->max_voltage_mv != 4350){
					chip->max_voltage_mv = 4350;
				}
			}else if(chip->max_voltage_mv != CHARGE_VOLATE){
					chip->max_voltage_mv = CHARGE_VOLATE;
			}
			pr_info("2.CV max_vol: %d\n",(chip->max_voltage_mv));
			#endif
...
#if 0
int qpnp_exp_hw_chg_therm_lim(struct qpnp_chg_chip *chip)
{
	int rc = 0;
	u8 batt_therm_lim = 0;
	rc = qpnp_chg_read(chip, &batt_therm_lim,
				 chip->bat_if_base + BAT_IF_BTC_CTRL, 1);
	if (rc) {
		pr_err("spmi read failed: addr=%03X, rc=%d\n",
				chip->bat_if_base + BAT_IF_BTC_CTRL, rc);
		return rc;
	}

	if((batt_therm_lim & 0x03) != 0x02)
		{

		batt_therm_lim &= ~0x01;
		batt_therm_lim |= 0x02;

		rc = qpnp_chg_write(chip, &batt_therm_lim,
		chip->bat_if_base + BAT_IF_BTC_CTRL, 1);
	}
	return 0;
}
				...
				struct spmi_resource *spmi_resource)
{
	...
	case SMBCL_CHGR_SUBTYPE:
		...
		reg = 0x0F;
        rc = qpnp_chg_write(chip, &reg, chip->chgr_base + 0x54, 1);
		break;
	case SMBB_BUCK_SUBTYPE:
	case SMBBP_BUCK_SUBTYPE:
	case SMBCL_BUCK_SUBTYPE:
	...
	case SMBCL_BAT_IF_SUBTYPE:
		...
		if (init_data->constraints.name) {
			...
			if (IS_ERR(chip->batfet_vreg.rdev)) {
               ...
               // qpnp_exp_hw_chg_therm_lim(chip);
		break;
	case SMBB_USB_CHGPTH_SUBTYPE:
	case SMBBP_USB_CHGPTH_SUBTYPE:
	case SMBCL_USB_CHGPTH_SUBTYPE:
...
qpnp_charger_read_dt_props(struct qpnp_chg_chip *chip)
{
    ...
    CHARGE_VOLATE = chip->max_voltage_mv;
