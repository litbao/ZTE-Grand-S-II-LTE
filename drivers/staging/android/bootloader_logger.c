...
static int bootloader_logger_init(struct bootloader_logger *logger, char *old_buf)
{
	...
	printk(KERN_INFO "bootloader_logger_%s: buffer=%p, size=%d\n", 
...
static int __devinit bootloader_logger_driver_probe(struct platform_device *pdev)
{
...
static int __init bootloader_logger_late_init(void)
{
	...
	if (logger_main.old_log != NULL) {
		entry = create_proc_entry("bootloader_log", S_IFREG | S_IRUGO, NULL);
		if (!entry) {
	...
	if (logger_last.old_log != NULL) {
		entry = create_proc_entry("bootloader_log_last", S_IFREG | S_IRUGO, NULL);
		if (!entry) {
