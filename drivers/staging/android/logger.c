...
DEFINE_LOGGER_DEVICE(log_kernel, "log_kernel", 1024*1024)
...
static DEFINE_SPINLOCK(kernel_log_lock);
static DECLARE_WAIT_QUEUE_HEAD(logger_kernel_wait);
static int log_pending;

static int logger_kernel_write(void *arg)
{
	struct logger_log *log_dst = &log_main;
	struct logger_log *log = &log_kernel;
	struct logger_reader *reader;
	size_t len, ret;
	unsigned long flags;

	pr_info("%s: logger thread start\n", __func__);
	
	do {
		wait_event_interruptible(logger_kernel_wait,  
			log_pending || kthread_should_stop());
		if (kthread_should_stop())
			break;

		mutex_lock(&log_dst->mutex);
		spin_lock_irqsave(&kernel_log_lock, flags);
		log_pending = 0;	
		list_for_each_entry(reader, &log->readers, list) {
		...
		spin_unlock_irqrestore(&kernel_log_lock, flags);
...
static int disable_kernel_logger;
module_param(disable_kernel_logger, int, 0600);
static struct task_struct *kernel_logger_task;
...
logger_console_write(struct console *console, const char *s, unsigned int count)
{
	...
	if (unlikely(disable_kernel_logger))
		return;
	...
	do_write_log(log, s, header.len - (sizeof(extendedtag)) - 1);
 
	/* the write offset is updated to add the final extra byte */
	log->w_off = logger_offset(log, log->w_off + 1);
	log_pending = 1;
	spin_unlock_irqrestore(&kernel_log_lock, flags);
	/* avoid deadlock when called from scheduler */
	if (kernel_logger_task && !preempt_count())
		wake_up_interruptible(&logger_kernel_wait);
}
...
static struct console logger_console = {
	...
	.flags	= CON_PRINTBUFFER | CON_ENABLED | CON_IGNORELEVEL,
	.index	= -1,
};

static int __init logger_thread_init(void)
{
	int rc;
	static struct task_struct *task;

	pr_info("%s: create logger thread\n", __func__);
	
	task = kthread_create(logger_kernel_write, NULL,
			   "logger-kernel");
	if (IS_ERR(task)) {
		rc = PTR_ERR(task);
		pr_err("%s: failed to start kenrel logger thread: %d\n", __func__, rc);
		return rc;
	}
	wake_up_process(task);
	kernel_logger_task = task;
	pr_info("%s: logger thread %p\n", __func__, task);

	return 0;
}
module_init(logger_thread_init)
...
static int __init logger_console_init(void)
{
	...
	pr_info("%s: register logger console\n", __func__);
