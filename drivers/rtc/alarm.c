...
static void update_timer_locked(struct alarm_queue *base, bool head_removed)
{
	...
	bool is_wakeup = base == &alarms[ANDROID_ALARM_RTC_WAKEUP] ||
			base == &alarms[ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP] ||
			base == &alarms[ANDROID_ALARM_RTC_POWERON_WAKEUP];
#endif
	if (base->stopped) {
...
static int alarm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	hrtimer_cancel(&alarms[ANDROID_ALARM_RTC_POWERON_WAKEUP].timer); 
	...
	tmp_queue = &alarms[ANDROID_ALARM_RTC_POWERON_WAKEUP];
	if (tmp_queue->first && (!wakeup_queue ||
				hrtimer_get_expires(&tmp_queue->timer).tv64 <
				hrtimer_get_expires(&wakeup_queue->timer).tv64))
		wakeup_queue = tmp_queue;
	if (wakeup_queue) {
		...
		if (rtc_current_time + 1 >= rtc_alarm_time) {
			...
			update_timer_locked(&alarms[ANDROID_ALARM_RTC_WAKEUP],
									false);
			update_timer_locked(&alarms[ANDROID_ALARM_RTC_POWERON_WAKEUP], false); 			
...
static void alarm_shutdown(struct platform_device *pdev)
{
	...
	printk("[rtc] alarm_shutdown \n");

 	hrtimer_cancel(&alarms[ANDROID_ALARM_RTC_POWERON_WAKEUP].timer);
 
	wakeup_queue = &alarms[ANDROID_ALARM_RTC_POWERON_WAKEUP];
 	if (wakeup_queue->first) {
		...
		pr_alarm(SUSPEND,
			...
			rtc_alarm_time, rtc_current_time,
			rtc_delta.tv_sec, rtc_delta.tv_nsec);
		printk("[rtc] rtc alarm set at %ld, now %ld, rtc delta %ld.%09ld\n",
			rtc_alarm_time, rtc_current_time,
			rtc_delta.tv_sec, rtc_delta.tv_nsec);
		if (rtc_current_time + 1 >= rtc_alarm_time) {
			pr_alarm(SUSPEND, "alarm about to go off\n");
			...
			wake_lock_timeout(&alarm_rtc_wake_lock, 2 * HZ);
			update_timer_locked(&alarms[ANDROID_ALARM_RTC_POWERON_WAKEUP],
									false);
		}
	}else{
			printk("[rtc] alarm_shutdown clear alarm \n");
			rtc_set_alarm_poweron(alarm_rtc_dev, NULL);
	}
}
#endif
