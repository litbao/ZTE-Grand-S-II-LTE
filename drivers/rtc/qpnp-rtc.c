...
static int
qpnp_rtc_set_alarm_poweron(struct device *dev, struct rtc_wkalrm *alarm)
{
	u8 value[4] = {0};
	u8 reg;
	int rc;
	...
	if (!alarm) {
		spin_lock_irqsave(&rtc_dd->alarm_ctrl_lock, irq_flags);
		dev_dbg(rtc_dd->rtc_dev, "Disabling alarm interrupts\n");
		...
		if (rc)
			dev_err(rtc_dd->rtc_dev, "SPMI write failed\n");
		printk("[rtc] qpnp_rtc_set_alarm_poweron clear alarm\n");
fail_alarm_disable:
		spin_unlock_irqrestore(&rtc_dd->alarm_ctrl_lock, irq_flags);
		return rc;
	}
	printk("[rtc] qpnp_rtc_set_alarm_poweron set alarm\n");
	return qpnp_rtc_set_alarm(dev, alarm);
}
...
static struct rtc_class_ops qpnp_rtc_ops = {
	...
	.set_alarm_poweron = qpnp_rtc_set_alarm_poweron,
...
unsigned int reg_alarm = 0;

#if 0
static ssize_t show_reg_alarm(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	return sprintf(buf, "%d\n", reg_alarm);
}

static struct device_attribute reg_alarm_attr =
	__ATTR(reg_alarm, S_IRUGO | S_IWUSR, show_reg_alarm, NULL);
#endif
		...
		const char *buff, size_t len, loff_t *off)
{
	...
	if (copy_from_user(messages, buff, len))
      ...
      // rtc_set_time(rtc_dev,&tm);
       ...
       printk("rtc_read_time    %02d:%02d:%02d %04d-%02d-%02d\n",
		...
		char __user *buff, size_t len, loff_t *off)
{
    ...
    rtc_len = sprintf(temp_buff, "%d\n", reg_alarm);

    if (pos >= rtc_len)
    {
        printk("rtc_proc_read end\n");
        return 0;
    }
    ...
    *off += count;
    printk("[rtc] rtc_proc_read reg_alarm =%d rtc_len =%d \n",reg_alarm,rtc_len);
...
static int create_rtc_proc_file(void)
{
    ...
    entry = create_proc_entry("driver/rtc_reg_alarm", S_IFREG | S_IRUGO, NULL);
    if (entry)
...
static int __devinit qpnp_rtc_probe(struct spmi_device *spmi)
{
	...
	enable_irq_wake(rtc_dd->rtc_alarm_irq);
	{
		...
		if (rc < 0) {
			dev_err(&spmi->dev, "RTC alarm time read failed\n");
		}

		reg_alarm = value[0] | (value[1] << 8) | (value[2] << 16) | \
						 (value[3] << 24);
		printk("[rtc] reg_alarm=%d \n",reg_alarm);

		/* create attr file 
		rc = device_create_file(&spmi->dev, &reg_alarm_attr);
		if (rc) {
			dev_err(&spmi->dev, "failed: create reg_alarm file\n");
		} */
	}
