...
static void mdss_fb_shutdown(struct platform_device *pdev)
{
    ...
    if (mfd && mfd->index == 0 && mfd->panel_power_on)
    {
        mutex_lock(&mfd->bl_lock);
...
void mdss_fb_update_backlight(struct msm_fb_data_type *mfd)
{
	...
	{
		pr_info("mdss_fb_update_backlight failed, splash_done_flag = 0\n");
		return;
	}
	if (!mfd->bl_updated)
		pr_info("%s, unset_bl_level = %d, bl_updated = %d frame_count = %d\n", __func__, unset_bl_level, mfd->bl_updated,
		mfd->panel_info->frame_count);
	/** ZTE_MODIFY end chenfei */
	if (unset_bl_level && !mfd->bl_updated) {
		pdata = dev_get_platdata(&mfd->pdev->dev);
		if ((pdata) && (pdata->set_backlight)) {
			mutex_lock(&mfd->bl_lock);
			mfd->bl_level = unset_bl_level;
			pdata->set_backlight(pdata, mfd->bl_level);
			mfd->bl_level_old = unset_bl_level;
			mutex_unlock(&mfd->bl_lock);
			mfd->bl_updated = 1;
		}
	}
...
static int mdss_fb_register(struct msm_fb_data_type *mfd)
{
	...
	/** ZTE_MODIFY chenfei added for LCD driver 2013/05/21 */ 
	//add frame_count debugfs node
	{
		struct dentry *msm_fb_debugfs_root = NULL;
		struct dentry *msm_fb_debugfs_file = NULL;
		...
		if (msm_fb_debugfs_root != NULL) {
				...
				sub_name[1] = '\0';
				sub_dir = debugfs_create_dir(sub_name, msm_fb_debugfs_root);
			} else {
				sub_dir = NULL;
			}


		msm_fb_debugfs_file = debugfs_create_u32("frame_count",  S_IRUGO | S_IWUSR, sub_dir,
							   (u32 *) &mfd->panel_info->frame_count);
	}
	/** ZTE_MODIFY end chenfei */
		...
		struct mdp_display_commit *disp_commit)
{
	...
	++mfd->panel_info->frame_count;/** ZTE_MODIFY chenfei added for LCD driver 2013/05/21 */ 
	return ret;
}
