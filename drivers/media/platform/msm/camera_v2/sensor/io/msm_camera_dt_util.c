	...
	*power_setting = ps;
	...
	for (i = 0; i < count; i++) {
		...
		case SENSOR_GPIO:
			...
			else if (!strcmp(seq_name, "sensor_gpio-ctrdvdd"))
				ps[i].seq_val = SENSOR_GPIO_CTRDVDD;
			else
				rc = -EILSEQ;
			break;
		case SENSOR_CLK:
	...
	uint16_t gpio_array_size)
{
	...
	if (of_property_read_bool(of_node, "qcom,gpio-ctrdvdd") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-ctrdvdd", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-ctrdvdd failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-ctrdvdd invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_CTRDVDD] =
			gpio_array[val];
		printk("%s qcom,gpio-ctrdvdd %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_CTRDVDD]);
	}
