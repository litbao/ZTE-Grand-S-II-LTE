...
// ZTE_MODIFY caoyaobin add temporary for building, please check it.
...
static struct i2c_driver imx132_i2c_driver = {
	...
	.probe  = msm_imx132_i2c_probe, // ZTE_MODIFY caoyaobin from msm_sensor_i2c_probe,
	.driver = {
...
static int imx132_read_proc_info(char *page, char ** start, off_t offset,
	int count, int *eof, void *data)
{
	*eof = 1;
	return sprintf(page, "ID: 0x132\n" "Name: imx132\n");
}

static int create_proc_camera_entity(void)
{
	if (create_proc_read_entry("front_camera_info",0, NULL,
	  	    imx132_read_proc_info, NULL) == NULL) {
	  	pr_err("%s:%d unable to create front camera proc info\n", __func__, __LINE__);
	}
	return 0;
}
