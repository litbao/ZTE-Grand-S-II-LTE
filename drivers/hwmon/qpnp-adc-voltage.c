...
struct qpnp_vadc_chip *vadc_tmp;
		...
		int count, int *eof, void *data)
{
    ...
    int rc = -1;
    int len=0;
    printk("zte_read_hver_material \n");
    rc = qpnp_vadc_read(vadc_tmp, LR_MUX9_PU2_AMUX_THM5, &result);
    if (rc) {
        pr_err("VADC read error with %d\n", rc);
        return 0;
    }

    if(result.physical > 67000 && result.physical < 107000){
        snprintf(page ,5,"BOM1");
        len = 5;
    }
    else if(result.physical > 173000 && result.physical < 213000){
        snprintf(page ,5,"BOM2");
        len = 5;
    }
    else if(result.physical > 280000 && result.physical < 320000){
        snprintf(page ,5,"BOM3");
        len = 5;
    }
	else if(result.physical > 363000 && result.physical < 403000){
        snprintf(page ,5,"BOM4");
        len = 5;
    }
    else if(result.physical > 658000 && result.physical < 698000){
        snprintf(page ,5,"BOM5");
        len = 5;
    }
	else if(result.physical > 709000 && result.physical < 749000){
        snprintf(page ,5,"BOM6");
        len = 5;
    }
    else if(result.physical > 880000 && result.physical < 920000){
        snprintf(page ,5,"BOM7");
        len = 5;
    }
	else if(result.physical > 987000 && result.physical < 1027000){
        snprintf(page ,5,"BOM8");
        len = 5;
    }
    else if(result.physical > 1123000 && result.physical < 1163000){
        snprintf(page ,5,"BOM9");
        len = 5;
    }
	else if(result.physical > 1180000 && result.physical < 1220000){
        snprintf(page ,6,"BOM10");
        len = 6;
    }
    else if(result.physical > 1389000 && result.physical < 1429000){
        snprintf(page ,6,"BOM11");
        len = 6;
    }
	else if(result.physical > 1467000 && result.physical < 1507000){
        snprintf(page ,6,"BOM12");
        len = 6;
    }
    else{
        snprintf(page ,4,"ERR\n");
